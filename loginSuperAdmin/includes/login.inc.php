...
if (isset($_POST['login-submit'])) {
	...
	if (empty($mail) || empty($password)) {
		header("Location: ../index.php?error=emptyfields=" . $mail);
		exit();
	} else {
		$sql = "SELECT * FROM superAdmin WHERE uidSAdmin=?;";
		...
		if (!mysqli_stmt_prepare($stmt, $sql)) {
			...
			if ($row = mysqli_fetch_assoc($result)) {
				...
				if ($pwdCheck == FALSE) {
					...
					header("Location: ../index.php?error=wrongpwd&pwd=" . $pwdCheck);
					exit();
					...
					$_SESSION['id'] = $row['idSAdmin'];
					$_SESSION['email'] = $row['uidSAdmin'];
